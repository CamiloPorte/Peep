...
func (s *server) FetchBeers(w http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(w).Encode(peep.FetchAnswer{
		Beers: allBeers,
	})
}
...
func (s *server) BeerDetail(w http.ResponseWriter, r *http.Request) {
	...
	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(peep.FetchByIdAnswer{
		BeerByID: *answ,
	})
...
func (s *server) BeerPrice(w http.ResponseWriter, r *http.Request) {
	...
	w.WriteHeader(http.StatusOK)
	json.NewEncoder(w).Encode(peep.PriceByBox{
		PriceBox: peep.BeerBox{
			TotalPrice: total,
		},
	})
